...
<head>
    ...
    <title>Manajemen Divisi - PT KAI</title>
                                ...
                                <tbody>
                                    @foreach($divisions as $index => $division)
                                        <tr>
                                            ...
                                            <td>{{ $division->office->office_name ?? '-' }}</td>
                                            <td>{{ $division->division_head ?? '-' }}</td>
                                            <td class="text-center">
                                                <input type="checkbox" class="toggle-status"
                                                    data-division-id="{{ $division->id }}"
                                                    {{ $division->division_status ? 'checked' : '' }}>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('divisi.show', $division->id) }}" class="btn btn-secondary btn-sm">
                                                    <i class="fas fa-eye"></i> Lihat
                                                </a>
                                                <a href="{{ route('divisi.edit', $division->id) }}" class="btn btn-info btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <button class="btn btn-danger btn-sm delete-division-btn"
                                                    data-toggle="modal"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="deleteDivisionModalLabel">
...
<script>
    $(document).ready(function () {
        ...
        $('.delete-division-btn').click(function () {
            ...
            let deleteUrl = "{{ url('division') }}/" + divisionId;
        ...
        $(".toggle-status").change(function () {
            ...
            $.post("{{ url('division') }}/" + divisionId + "/toggle-status", {
                _token: '{{ csrf_token() }}',
                status: status
            }, function (res) {
                if (res.success) {
